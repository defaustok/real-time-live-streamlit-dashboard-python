...
while True:
    ...
    df['Short PnL'] = (df['Unrealized_PnL_%'] + df['Short_PNL']) * initial_balance #PnL of short position in USD
    df['Short PnL%'] = (df['Unrealized_PnL_%'] + df['Short_PNL']) #PnL of short position in %
    ...
    with placeholder.container():
        current_b, initial_b , a, b = st.columns(4)
        initial_b.metric(label = 'Initial Balance', value = f"$ {round(initial_balance,2)}")
        current_b.metric(label = 'Balance', value = f"$ {round(df['Overall Return_usd'].iloc[-1] + initial_balance,2)} ", delta = f"$ {round((df['Overall Return_usd'].iloc[-1] + initial_balance)-(df['Overall Return_usd'].iloc[-2] + initial_balance),2)}")
        ...
        with fig_col3:
            ...
            fig3.add_trace(
                go.Scatter(x=df.index, y=(df['Short PnL%'] + df['Pool_Money']), name="Sum PnL, %"))
        ...
        with fig_col5:
            ...
            fig5.add_trace(
                go.Scatter(x=df.index, y=df['Short PnL%'] * initial_balance, name="Short PnL, USD"))
            fig5.add_trace(
                go.Scatter(x=df.index, y=((df['Short PnL%'] + df['Pool_Money'])*initial_balance), name="Sum PnL, USD"))
